...
function menuItemKeyDown(evt) {
   ...
   if ( item != null ) {
      ...
      case 13: // enter key
         ...
         break;
      default: // Navigating with first letter of menu option 
		 checkKey(evt.keyCode,item);
         break;
      }
   }
}

// Checks if the key pressed matches the first letter of a menu option
function checkKey(keyCode,item){	
	var keypress = String.fromCharCode(keyCode),
		keypressLowerCase = keypress.toLowerCase(),
		// Check for position of first char in option
		firstCharPos = getStartCharPosition(item),
		// Get first letter of option
		currentPositionFirstChar = item.text.charAt(firstCharPos),
		items = item.parentMenu.items,
		textContent;
	
	// If key pressed is the same as the first char of the currently selected option
	if(keypressLowerCase==currentPositionFirstChar.toLowerCase()){
		// Find the next option beginning with the same letter
		var newPosItem = checkNextMatch(items, items.indexOf(item), currentPositionFirstChar, keypress);
		if((newPosItem!=item)&&(newPosItem.isEnabled)){			
			changeFocusAfterKeypress(newPosItem);
			
		}
	}
	else { // Search for option begininng with key pressed
		for(var k=0, nal = items.length; k < nal; k++){
			firstCharPos = getStartCharPosition(items[k]);
			textContent = items[k].text;
			// If key pressed matches first letter of menu option and option is enabled
			if((keypressLowerCase == textContent.charAt(firstCharPos).toLowerCase())&&(items[k].isEnabled)){										
				changeFocusAfterKeypress(items[k]);
				break;
			}			    		
		}
	}
}
//This functions finds the next menu option beginning with a specified key, if the current is the same
function checkNextMatch(items,currentPosition,currentPositionFirstChar,keypress){     
	var firstCharPos,
		looped = false,
		startChar,
		loopLimit = items.length,
		loopCount = (currentPosition == loopLimit - 1) ? -1 : currentPosition;
	
	for (var i = loopCount+1; i < loopLimit; i++){			
		firstCharPos = getStartCharPosition(items[i]);
		startChar = items[i].text.charAt(firstCharPos).toLowerCase();
		if(startChar == keypress.toLowerCase()&&(items[i].isEnabled)){
				currentPosition = i;
				break;
		}
		// If reached end of array, loop through it again up to starting point
		if(i == items.length-1){
			// If have looped through it already, stop
			if(looped){ 
				break;
			}
			i = -1; // Reset counter
			loopLimit = currentPosition;
			looped = true;
		}
...
//Find the position of the first char in a string - 0 in ltr languages, otherwise last
function getStartCharPosition(item){
	var ltr = item.parentMenu.isLTR;
	if((item.text)&&(!ltr)){
...
//Moves focus to newly selected option after keypress
function changeFocusAfterKeypress(item){
	if ( item != null ) {
		if ( !item.isSelected ) {
			// set focus on the anchor and select the menu item
			item.anchorTag.focus();
		}
		if ( item.submenu != null && !item.submenu.isVisible && (item.isEnabled || item.isDefault)) {
			// show the submenu
...
function handleMenuLaunchKeys( name, launcher, event ) {
	// If enter, space, up or down arrow pressed, launch menu
	if((event.keyCode==13)||(event.keyCode==32)||(event.keyCode==38)||(event.keyCode==40))
	{
		event.preventDefault();
		showContextMenu( name, launcher );
	}	
}
